...
function update() {
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        // SkyObstacle은 checkCollision 인자가 적으나 에러는 발생하지 않음
...
function drawObstacles() {
    for (let obs of obstacles) {
        if (obs.draw) {
            obs.draw(ctx, canvas.height); // SkyObstacle uses only ctx, canvas.height is ignored
        }
    }
}
...
// === Events ===
btnSky?.addEventListener('click', () => startGame());
